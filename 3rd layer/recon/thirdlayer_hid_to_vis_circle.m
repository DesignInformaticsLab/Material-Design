...
addpath('utils','function_code','hidstate_circle_3rdlayer_p2p2_(96f96f144f12ws18ws18wsPb01)')
% One=load('WB_real_alloy_w24_b02_rot_nrot24_pb0.1_pl0.1_iter_500.mat');
One=load('circle_nonor_ws12_f96_alloy_w12_b96_rot_nrot1_pb0.1_pl10_iter_3000.mat');
...
%     hidstate_2nd_layer=zeros(96,20736);
%     hidstate_pool2=zeros(72,72,96);
...
%     hidstate=reshape(double(hidstate),[288 1 1296]);
    ...
    fname=sprintf('hidstates3rd_circle_(p2p2)_(96f96f144f12ws18ws18wsPb01)_%d',ii); %WB
...
%     hidstate=negdata;
    ...
    temp_binary=zeros(72,72,96);
    [svals,idx]=sort(negdata(:),'descend');
    thre=svals(round(72*72*96*0.1));
...
%         temp_fdog2(Gmag>300)=1;
    ...
    hidstate=reshape(hidstate,[1,96,(L3*2)^2]);
...
%         negdata = sigmoid(negdata);
    ...
    [svals,idx]=sort(negdata(:),'descend');
    thre=svals(round(numel(negdata)*0.1));
    temp_binary=zeros(144,144,96);
    for aa = 1:size(negdata,3)
        temp_fdog2=reshape(negdata(:,:,aa),[144 144]);
...
%         end
    ...
    hidstate=reshape(hidstate,[288 288 96]);


hidstate=permute(hidstate,[3,1,2]);
hidstate=reshape(hidstate,[1,96,(L3*4)^2]);    
