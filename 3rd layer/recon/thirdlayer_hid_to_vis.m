...
addpath('utils','function_code','hidstate_3rdlayer_p2p2_(2f40f24f6ws9ws18ws12rP20P10P10Pb01)')
...
fname=sprintf('3rd_POOL2_(real_bibi)_(2f40f24f6ws9ws9ws12rP20P10P40Pb01)_alloy_w9_b24_trans_ntx1_gr1_pb0.1_pl10_iter_1000');
...
for ii = 1:100
    ...
    fname=sprintf('hidstates3nd_WB_nowh(p2p2)_(2f40f24f6ws9ws18ws12rP20P10P10Pb01)_%d',ii); %WB
    load([fname '.mat'],'hidstate');
    %% 3rd layer hidstate pool back by2
    for pp=1:size(hidstate,1)
        hidstate=reshape(hidstate,[24 729]);
    ...
    hidstate=reshape(hidstate,[24 1 2916]);
   %% 3rd layer reconstruction (pool back by 2)
    ...
    for nf = 1:params_Three.numhid
        for nt = 1:params_Three.numtx
            filter_t = W_Three(:,nf);
            filter_t = reshape(filter_t,[ws_Three*2,ws_Three*2,numchannels]);
    ...
    hidstate=reshape(hidstate,[1,40,L3^2]);
...
negdata = zeros(L3, H3, numchannels);
    for nf = 1:params_Two.numhid
        for nt = 1:params_Two.numtx
            ...
            S = reshape(hidstate(nt,nf,:),L3,H3);
            S1 = S;
            S1=im2bw(S1);
            S1=double(S1);
            filter_x = ones(2,2);
...
%% 2nd layer hidstate pool back by 2
    ...
    hidstate=reshape(hidstate,[108 108 24]);


hidstate=permute(hidstate,[3,1,2]);
hidstate=reshape(hidstate,[12,2,(L3*2)^2]);    
