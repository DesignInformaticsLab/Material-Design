...
addpath('utils','function_code','hidstate_2ndlayer_circle_(96f96f12ws18ws)')
...
W=gather(W.vishid);
for i = 1:size(W,2)
    W_temp=reshape(W(:,i),[31104/96 96]);
    for j = 1:96
...
L2=400-12+1;H2=L2;L1=356;H1=356;
...
for ii = 1:20
    ...
    fname=sprintf('hidstate_2ndlayer(1stp2)_circle_(96f96f12ws18ws)_%d',ii); %WB
    load([fname '.mat'],'hidstate');
    
    %% pool back hidstate
    for pp=1:size(hidstate,1)
        hidstate=reshape(hidstate,[96 178*178]);
    ...
    hidstate=reshape(hidstate_2nd_layer,[96 1 L1*L1]);    
        
    %% 2nd layer reconstruction    
    numchannels=params_Two.numch;
    negdata = zeros(L2-ws_Two*2+1+2, H2-ws_Two*2+1+2, numchannels);
    for nf = 1:params_Two.numhid
        for nt = 1:params_Two.numtx
            ...
            S = reshape(hidstate(nf,nt,:),L2-ws_Two*2+1+2,H2-ws_Two*2+1+2);
...
hidstate=reshape(hidstate,[1,96,L1^2]);
